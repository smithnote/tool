...
bool HZ2PY::toPinYin(const std::string &src, std::vector<std::string> &res_vec) const {
    if (src.empty()) {
        return false;
    }
    for (size_t i = 0; i < src.size();) {
        Unicode unicode;
        if (!toUnicode(src, i, unicode)) {
            continue;
        }
        std::vector<std::string> py_vec;
        if (!getPinYinVec(unicode, py_vec) || py_vec.empty()) {
            continue;
        }
        res_vec.push_back(py_vec.front());
    }
    return true;
}
...
bool HZ2PY::toUnicode(const std::string &src, size_t &index, Unicode &result) const {
    if (src.empty() || index >= src.size()) {
        return false;
    }
    result = 0;
...
bool HZ2PY::getPinYinVec(const Unicode unicode,
                         std::vector<std::string> &result_vec,
                         bool with_tone) const {
...
const char* HZ2PY::pinyin_with_tone_vec_[] = {
    ...
    "hong2",
    "qi2 ji1",
    "qian1 fen1 zhi1 yi1 gong1 sheng1",
    "gong1 sheng1",
    "shi2 fen1 zhi1 yi1 gong1 sheng1",
    "shu4",
    "miao4",
    "ju3",
    "zhan4",
